...
function visitElement(generator, block, state, node, elementStack, componentStack) {
    ...
    if (!childState.namespace && node.canUseInnerHTML && node.children.length > 0) {
        if (node.children.length === 1 && node.children[0].type === 'Text') {
            block.builders.create.addLine(name + ".textContent = " + stringify(node.children[0].data) + ";");
        }
        else {
            block.builders.create.addLine(name + ".innerHTML = " + stringify(node.children.map(toHTML).join('')) + ";");
        }
    }
    else {
        node.children.forEach(function (child) {
